...
const Product = ({ product }) => {
  ...
  return (
    <div className='border-2 border-gray-900 rounded-lg
      ...
      <h2 className='text-center font-bold text-sm sm:text-base lg:text-lg line-clamp-2 px-2 mt-2'>
        {product.name}
      </h2>

      <p className='text-center text-green-700 font-semibold text-sm sm:text-base lg:text-lg mt-1'>
        ₹{product.price}
      </p>

      {message && (
        <p className={`text-center text-xs mt-1 px-2 ${
      ...
      <div className='flex flex-col gap-1.5 sm:flex-col justify-center text-xs sm:text-sm mt-auto m-1 pt-2'>
        <button
          className={`text-white px-2 py-1.5 rounded-lg transition whitespace-nowrap text-xs sm:text-sm w-full ${
            product.stock === 0
